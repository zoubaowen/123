...
export class CloudbaseAgentService {
  ...
  private async launchAgent(
    ...
    try {
      ...
      // ── 判断是否被用户取消 ──
      const wasCancelled = abortController.signal.aborted || getAgentRun(conversationId)?.status === 'cancelled'
      ...
      setTimeout(() => {
        persistenceService
          .cleanupStreamEvents(conversationId, assistantMessageId, userContext.envId, userContext.userId)
          .catch(() => {
            // Non-critical
          })
      }, 600)
