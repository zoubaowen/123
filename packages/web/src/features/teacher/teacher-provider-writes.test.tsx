...
function apiSource(writer: Partial<TeacherWorkspaceWriter> = {}, data: TeacherDashboardData = dataWithCourse()) {
  ...
  const source: TeacherWorkspaceSource = {
    ...
    writer: {
      ...
      createChapter: vi.fn(async () => ({ ok: true }) as TeacherWriteResult),
      createLesson: vi.fn(async () => ({ ok: true }) as TeacherWriteResult),
      createResource: vi.fn(async () => ({ ok: true }) as TeacherWriteResult),
