...
import { AlertCircle, Trash2, GitBranch, Loader2, Search, X, MoreVertical } from 'lucide-react'
...
import { filterStudentCreations, StudentSidebarNav } from '@/features/student-workspace/student-sidebar-nav'
...
export function TaskSidebar({ tasks, width = 288 }: TaskSidebarProps) {
  ...
  const filteredStudentTasks = useMemo(
    () => filterStudentCreations(tasks, creationSearchQuery),
    [creationSearchQuery, tasks],
  )
  ...
  if (!session.user) {
    return (
      ...
      >
        <StudentSidebarNav
          recentCreationCount={0}
          onNewCreation={handleNewTaskClick}
          onCoursePrompt={setTaskPrompt}
        />
  ...
  return (
    ...
    >
      <StudentSidebarNav
        recentCreationCount={filteredStudentTasks.length}
        onNewCreation={handleNewTaskClick}
        onCoursePrompt={setTaskPrompt}
        searchQuery={creationSearchQuery}
        onSearchQueryChange={setCreationSearchQuery}
      />
          ...
          {filteredStudentTasks.length === 0 ? (
            <Card>
              <CardContent className="p-3 text-center text-xs text-muted-foreground">
                {creationSearchQuery.trim()
                  ? `没有找到与“${creationSearchQuery.trim()}”相关的创作`
                  : '还没有任务，点击上方按钮创建第一个作品吧！'}
              </CardContent>
            </Card>
          ) : (
            <>
              {filteredStudentTasks.slice(0, 10).map((task) => {
      ...
      <AlertDialog open={showDeleteDialog} onOpenChange={setShowDeleteDialog}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>请选择要删除的任务类型。删除后无法恢复。</AlertDialogDescription>
          </AlertDialogHeader>
          <div className="py-4">
